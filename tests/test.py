...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.delay = 3
        self.properties = (
            "videoid title length duration author " "username category thumb published"
        ).split()
        self.runOnce()
    ...
    def test_generate_filename_with_meta(self):
        ...
        self.assertEqual(
            filename,
            "Jessie J - WILD (Official) ft. Big Sean"
            ", Dizzee Rascal-jrNLsC_Y9Oo-251.webm",
        )
        self.assertEqual(a.threed, False)
        self.assertEqual(
            a.title, "Jessie J - WILD (Official) ft. Big Sean" ", Dizzee Rascal"
        )
        self.assertEqual(a.notes, "DASH audio")
        self.assertEqual(
            a.filename, "Jessie J - WILD (Official) ft. Big Sean" ", Dizzee Rascal.webm"
        )
    ...
    def test_pafy__invalid_win_filename(self):
        ...
        expected = (
            "Jon Meacham_ _Thomas Jefferson_ the Art of Power_ _ Talks" " at Google.m4a"
        )
    ...
    def test_video_properties(self):
        ...
        for video in Test.videos:
            ...
            self.assertEqual(
                hashlib.sha1(description).hexdigest(), video["description"]
            )
